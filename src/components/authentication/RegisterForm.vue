...
async function register() {
  try {
    const response = await axios.post(
      "https://zulkit.viainvitation.com/api/register", {
...
<template>
    <form>
        <div class="mb-4">
            ...
            <input @keyup.enter="register" v-model="form.name" placeholder="Type your full name" id="name" type="text" name="name"
        ...
        <div class="mb-4">
            ...
            <input @keyup.enter="register" v-model="form.email" placeholder="Type your email" id="email" type="text" name="email"
                class="block w-full py-3 mt-2 border border-gray-300 rounded-full shadow-sm px-7 focus:border-indigo-300 focus:outline-none focus:ring focus:ring-indigo-200 focus:ring-opacity-50 disabled:bg-gray-100" />
        </div>
        <div class="mb-4">
            <label class="block mb-1" for="password">Password</label>
            <input @keyup.enter="register" v-model="form.password" placeholder="Type your password" id="password" type="password" name="password"
                class="block w-full py-3 mt-2 border border-gray-300 rounded-full shadow-sm px-7 focus:border-indigo-300 focus:outline-none focus:ring focus:ring-indigo-200 focus:ring-opacity-50 disabled:bg-gray-100" />
        </div>
        <div class="mt-6">
            <button
